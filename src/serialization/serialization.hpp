//NOTE: Major security vulernability - during serialization of containers, if the container length is specifified as 
//greater than the rest of the serialization string, it'll read undefined bytes.
...
namespace serialization
{
	...
	{
		inline serialization_state(uint32_t serialization_version);
		inline ~serialization_state()
		{
			free(buffer);
		}
		char* buffer;
	...
	{
		inline unserialization_state(char* string);
		char* serialized;
	...
	serialization_state::serialization_state(uint32_t serialization_version)
	{
		bufferUsed = 0;
	...
	unserialization_state::unserialization_state(char* string)
	{
		...
		serializedVersion = unserialize_basic<uint32_t>(*this);
	}
	...
	{
		return std::is_pointer<data>::value;
	}
	...
	template<class data>
	constexpr typename std::enable_if<std::is_arithmetic<data>::value && !is_pointer<data>(),void>::type serialize(data serializeMe, serialization_state& state)
	{
		return serialize_basic(serializeMe,state);
	}
	template<class data>
	constexpr typename std::enable_if<std::is_arithmetic<typename std::remove_pointer<data>::type>::value && is_pointer<data>(),void>::type serialize(data serializeMe, serialization_state& state)
	{
		return serialize_basic(*serializeMe,state);
	}
	...
	template<class data>
	constexpr typename std::enable_if<std::is_arithmetic<typename std::remove_pointer<data>::type>::value,data>::type unserialize(unserialization_state& state)
	{
		auto result = unserialize_basic<typename std::remove_pointer<data>::type>(state);
		if constexpr(is_pointer<data>())
		{
	...
	template <class data>
	constexpr typename std::enable_if<std::is_function<decltype(std::remove_pointer<data>::type::unserialize)>::value && is_pointer<data>(),data>::type unserialize(unserialization_state& state)
	{
	...
	template <class data>
	constexpr typename std::enable_if<get_container_classification<typename std::remove_pointer<data>::type>() != not_a_container && is_pointer<data>(),data>::type unserialize(unserialization_state& state)
	{
		constexpr auto classification = get_container_classification<typename std::remove_pointer<data>::type>();
		if constexpr(classification == sequence)
		{
	...
	/*!
	 * This template is for non-pointer values -- to which we will unserialize as a pointer, copy to the stack, then return.
	 */
	template<class data>
	typename std::enable_if<!is_pointer<data>() && !std::is_arithmetic<data>::value,data>::type unserialize(unserialization_state& state)
	{
		data* pointer_to_returnval = unserialize<typename std::add_pointer<data>::type>(state);
		data returnval = *pointer_to_returnval;
		delete pointer_to_returnval;
		return returnval;
	}
	...
	void serialize_basic(basicDataType serializeMe, serialization_state& state)
	{
		...
		for (size_t i=0;i<sizeof(serializeMe);i++)
		{
			...
			size_t it = i;
#else 
			size_t it = (sizeof(serializeMe)-i)-1;
#endif 
			state.buffer[state.bufferUsed+i] = bytes[it];
		}
	...
	basicDataType unserialize_basic(unserialization_state& state)
	{
		...
		basicDataType res = 0x0; //TODO

		//bytes[state.bufferUnserialized] = 0x0f;
		for (size_t i=0;i < length;i++)
		{
			...
			size_t it = i;
#else 
			size_t it = ((length-i)-1);
#endif 
			//bytes[it] = state.serialized[i+(state.bufferUnserialized)];
		
			res |= (static_cast<unsigned char>(state.serialized[i+state.bufferUnserialized]) << (8*((length-i)-1)) ); //TODO
